CREATE OR REPLACE FUNCTION public.list_courses()
 RETURNS TABLE(id uuid, name text, year integer, is_current boolean, started_at timestamp with time zone, ended_at timestamp with time zone)
 LANGUAGE sql
...
AS $function$
  ...
  WHERE public.has_role(auth.uid(),'admin')
     ...
     OR public.has_role(auth.uid(),'halaqah')
  ORDER BY is_current DESC, started_at DESC;
$function$;

CREATE OR REPLACE FUNCTION public.export_my_students_points(_course_id uuid)
...
AS $function$
  ...
  WHERE c.id = _course_id
    ...
    AND public.has_role(auth.uid(),'halaqah')
  ORDER BY 14 DESC, s.full_name;
$function$;

CREATE OR REPLACE FUNCTION public.export_my_students_recitations(_course_id uuid)
